...
R6 Schema version match
   ...
   validator's MOR_KIT_SCHEMA_VERSION.
EOF
}
...
check_R6() {
    ...
    if [[ "$sv" != "$MOR_KIT_SCHEMA_VERSION" ]]; then
        echo "✗ R6 — .meta.json schema_version=$sv does not match validator=$MOR_KIT_SCHEMA_VERSION" >&2
        return 1
    fi
}
