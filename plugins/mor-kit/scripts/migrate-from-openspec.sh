#!/usr/bin/env bash
# migrate-from-openspec.sh — convert legacy openspec/changes/ to mor-kit/changes/
# (or to ${MOR_KIT_ROOT}/<name>/ if env override set).
#
# Usage:
...
#   - If primary mor-kit/changes/ already exists with content → refuse, exit 1.
#   - Otherwise, move openspec/changes/ → mor-kit/changes/ (preserving archive/ subfolder).
#   - Add .mor-kit marker in primary root.
...
Migrates legacy openspec/changes/ to ${MOR_KIT_ROOT:-mor-kit/changes}/.
...
echo ""
echo "Next: re-run /superpowers:executing-plans or /mor-kit:review to verify the migration."
