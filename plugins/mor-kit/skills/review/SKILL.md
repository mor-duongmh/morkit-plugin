---
name: review
description: Generate the developer review checklist (human gate) for a mor-kit change, blocking implementation skills until the human flips "Overall Decision: OK". Triggered by /mor-kit:review or after /mor-kit:propose finishes.
license: MIT
---

Generate or refresh the **developer review checklist** for a mor-kit change. This file is the human gate that the plugin's PreToolUse hook checks before allowing implementation skills to run.
...
- `[change-name]` — optional; otherwise pick the most recently modified change folder under `${MOR_KIT_ROOT:-mor-kit/changes}/` (excluding `archive/`).
...
1. **Resolve change directory.**
   ...
   - If a name was provided, set `CHANGE_DIR="${MOR_KIT_ROOT:-mor-kit/changes}/<name>"` and verify it exists.
   ...
   - If no change folder exists, stop and tell the user to run `/mor-kit:propose` first.
...
3. **Report status to the user.**
   ...
   - A clear instruction to:
     ...
     5. Re-run `/superpowers:executing-plans` (or `/superpowers:executing-plans` / `/superpowers:subagent-driven-development`) — the plugin's hook will allow it.
...
Next steps:
  ...
  4. Run /superpowers:executing-plans (or /superpowers:executing-plans / /superpowers:subagent-driven-development)
