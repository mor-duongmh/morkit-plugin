...
case_1_1() {
    ...
    assert_dir_exists "mor-kit/changes/add-csv-export" "1.1 dir created"
    assert_file_exists "mor-kit/changes/add-csv-export/proposal.md" "1.1 proposal"
    assert_file_exists "mor-kit/changes/add-csv-export/design.md" "1.1 design"
    assert_file_exists "mor-kit/changes/add-csv-export/tasks.md" "1.1 tasks"
    assert_file_exists "mor-kit/changes/add-csv-export/.meta.json" "1.1 meta"
    assert_file_exists "mor-kit/changes/.mor-kit" "1.1 marker"
    cd /; rm -rf "$tmp"
}
...
case_1_2() {
    local tmp; tmp="$(mktemp -d)"; cd "$tmp" || return 1
    bash "$SCAFFOLD" add-csv-export >/dev/null 2>&1
    local meta; meta=$(cat mor-kit/changes/add-csv-export/.meta.json 2>/dev/null)
...
case_1_3() {
    ...
    proposal=$(cat mor-kit/changes/add-csv-export/proposal.md)
    tasks=$(cat mor-kit/changes/add-csv-export/tasks.md)
    design=$(cat mor-kit/changes/add-csv-export/design.md)
...
case_1_10() {
    ...
    assert_dir_exists "mor-kit/changes/foo" "1.10 plugin folder created"
    cd /; rm -rf "$tmp"
}

# ---------------------------------------------------------------------------
# Case 1.11 — MOR_KIT_ROOT env override
# ---------------------------------------------------------------------------
case_1_11() {
    local tmp; tmp="$(mktemp -d)"; cd "$tmp" || return 1
    MOR_KIT_ROOT=mor/changes bash "$SCAFFOLD" foo >/dev/null 2>&1
    assert_dir_exists "mor/changes/foo" "1.11 honored MOR_KIT_ROOT"
    assert_dir_not_exists "mor-kit/changes/foo" "1.11 default path skipped"
    assert_file_exists "mor/changes/.mor-kit" "1.11 marker in custom path"
    cd /; rm -rf "$tmp"
}
...
case_1_12() {
    ...
    assert_file_exists "mor-kit/changes/foo/proposal.md" "1.12 templates rendered"
    cd /; rm -rf "$tmp"
}
...
case_1_14() {
    local tmp; tmp="$(mktemp -d)"; cd "$tmp" || return 1
    bash "$SCAFFOLD" foo >/dev/null 2>&1
    local meta_at; meta_at=$(jq -r '.created_at' mor-kit/changes/foo/.meta.json 2>/dev/null)
...
case_1_15() {
    local tmp; tmp="$(mktemp -d)"; cd "$tmp" || return 1
    bash "$SCAFFOLD" foo >/dev/null 2>&1
    echo "manual edit" > mor-kit/changes/foo/proposal.md
    bash "$SCAFFOLD" --force foo >/dev/null 2>&1
    local rc=$?
    assert_equal "$rc" 0 "1.15 --force succeeds"
    local proposal; proposal=$(cat mor-kit/changes/foo/proposal.md)
...
case_1_16() {
    ...
    assert_dir_exists "mor-kit/changes/foo" "1.16 dir created"
    cd /; rm -rf "$tmp"
}
