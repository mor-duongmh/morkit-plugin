...
run_e2e() {
    ...
    assert_dir_exists "mor-kit/changes/add-csv-export" "e2e: scaffold ok"
    ...
    cat > mor-kit/changes/add-csv-export/review-checklist.md <<'EOF'
# Review Checklist
- [ ] Items
...
EOF
    ...
    cat > mor-kit/changes/add-csv-export/tasks.md <<'EOF'
# add-csv-export
...
EOF
    ...
    bash "$VALIDATE" mor-kit/changes/add-csv-export/tasks.md >/dev/null 2>&1
    assert_equal "$?" 0 "e2e: validate passes"

    # --- Gate blocks (PENDING) ---
    printf '%s' '{"tool_name":"Skill","tool_input":{"skill":"superpowers:executing-plans"}}' \
    ...
    sed -i.bak 's/Overall Decision: PENDING/Overall Decision: OK/' \
        mor-kit/changes/add-csv-export/review-checklist.md
    rm -f mor-kit/changes/add-csv-export/review-checklist.md.bak
    ...
    mv mor-kit/changes/add-csv-export mor-kit/changes/archive/add-csv-export
    assert_dir_exists "mor-kit/changes/archive/add-csv-export" "e2e: archived"
    assert_dir_not_exists "mor-kit/changes/add-csv-export" "e2e: active gone"
    ...
    assert_dir_exists "mor-kit/changes/legacy" "e2e: migration succeeded"
