...
# 4.10 — works with mor-kit/changes/<name>/
case_4_10() {
    ...
    bash "$GEN" mor-kit/changes/foo --variant BE-Feature >/dev/null 2>&1
    local rc=$?
    if [[ "$rc" -eq 0 ]]; then
        assert_file_exists "mor-kit/changes/foo/review-checklist.md" "4.10 review-checklist created"
        local content; content=$(cat mor-kit/changes/foo/review-checklist.md 2>/dev/null)
...
# 4.11 — usage mentions MOR_KIT_ROOT
case_4_11() {
    local out; out=$(bash "$GEN" --help 2>/dev/null)
    assert_contains "$out" "MOR_KIT_ROOT" "4.11 help mentions env override"
}
