...
# active mor-kit change's review-checklist.md is missing or not marked
# "Overall Decision: OK".
#
...
#
# Folder convention: ${MOR_KIT_ROOT:-mor-kit/changes}/
...
#   - no mor-kit/changes/ AND no openspec/changes/
...
# Resolve folder convention. Honor MOR_KIT_ROOT override.
...
if [[ -d "$CWD/$PRIMARY" ]]; then
    ...
    # Project doesn't use mor-kit plugin → fail-open
    exit 0
fi
...
✗ Refusing $skill_name for change "$CHANGE_NAME": review checklist missing.
  ...
  Run /mor-kit:review to generate the checklist (auto-detects BE/FE × Feature/BugFix/Refactor),
