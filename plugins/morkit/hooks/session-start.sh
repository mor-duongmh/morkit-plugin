#!/usr/bin/env bash
# morkit plugin — SessionStart hook (v2)
#
...
This project has legacy \`openspec/changes/\` from morkit plugin v1 but no \`$PRIMARY/\`.

In your first reply, ask the user:
"morkit plugin v2 đã đổi convention sang \`$PRIMARY/\`. Phát hiện \`openspec/changes/\` cũ.
