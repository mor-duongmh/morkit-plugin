...
case_6_1() {
    ...
    assert_dir_exists "morkit/changes/foo" "6.1 migrated"
    assert_file_exists "morkit/changes/foo/proposal.md" "6.1 content preserved"
    assert_file_exists "morkit/changes/.morkit" "6.1 marker created"
...
case_6_2() {
    ...
    bash "$MIG" >/dev/null 2>&1
    assert_dir_exists "morkit/changes/active" "6.2 active migrated"
    assert_dir_exists "morkit/changes/archive/old" "6.2 archive migrated"
    cd /; rm -rf "$tmp"
}
...
case_6_3() {
    ...
    assert_dir_not_exists "morkit/changes" "6.3 nothing created"
    cd /; rm -rf "$tmp"
}
...
case_6_6() {
    ...
    assert_dir_not_exists "morkit/changes" "6.6 dest not created"
    cd /; rm -rf "$tmp"
}
...
case_6_7() {
    ...
    assert_dir_exists "morkit/changes/foo" "6.7 migrated"
    cd /; rm -rf "$tmp"
}
...
case_6_8() {
    local tmp; tmp="$(mktemp -d)"; cd "$tmp" || return
    mkdir -p openspec/changes/foo
    mkdir -p morkit/changes        # empty primary
    bash "$MIG" >/dev/null 2>&1
    local rc=$?
    assert_equal "$rc" 0 "6.8 empty primary OK"
    assert_dir_exists "morkit/changes/foo" "6.8 migrated"
    cd /; rm -rf "$tmp"
}
