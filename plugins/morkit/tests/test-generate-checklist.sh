...
# 4.10 — works with morkit/changes/<name>/
case_4_10() {
    ...
    bash "$GEN" morkit/changes/foo --variant BE-Feature >/dev/null 2>&1
    local rc=$?
    if [[ "$rc" -eq 0 ]]; then
        assert_file_exists "morkit/changes/foo/review-checklist.md" "4.10 review-checklist created"
        local content; content=$(cat morkit/changes/foo/review-checklist.md 2>/dev/null)
