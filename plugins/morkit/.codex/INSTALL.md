...
`install-codex.sh` symlink `skills-codex/` + AGENTS.md, hỏi (interactive) có bật hooks hay không. Re-runnable an toàn.

Flags: `--yes` (accept defaults, hooks OFF), `--with-hooks` (force enable hooks via `hooks-codex.json`), `--uninstall` (remove symlinks).

> **Bootstrap note**: installer expects `skills-codex/` + `commands-codex/` đã tồn tại trong checkout. Nếu mới `git clone` xong và thấy lỗi "skills-codex/ not found", chạy `bash ~/.codex/morkit-source/plugins/morkit/scripts/sync-codex-fork.sh` để generate fork từ `skills/` + `commands/`.
...
2. **Symlink skills (bắt buộc):**
   Target là `skills-codex/` (Codex fork — đã rewrite `${CLAUDE_PLUGIN_ROOT}` → `${MORKIT_PLUGIN_ROOT}` và vocab khác), **không phải** `skills/` (Claude Code source).
   ```bash
   mkdir -p ~/.agents/skills
   ln -s ~/.codex/morkit-source/plugins/morkit/skills-codex ~/.agents/skills/morkit
   ```

   **Windows (PowerShell):**
   ```powershell
   New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.agents\skills"
   cmd /c mklink /J "$env:USERPROFILE\.agents\skills\morkit" "$env:USERPROFILE\.codex\morkit-source\plugins\morkit\skills-codex"
   ```

   Nếu `skills-codex/` không tồn tại trong checkout, chạy trước: `bash ~/.codex/morkit-source/plugins/morkit/scripts/sync-codex-fork.sh`.
...
4. **(Optional) Enable hooks:**
   Morkit's `SessionStart` + `PreToolUse` hooks **không auto-load** trong Codex 0.120.0. Cách nhanh nhất là dùng installer flag (tự `cp` / symlink `hooks-codex.json` + enable feature flag):
   ...
   ln -s ~/.codex/morkit-source/plugins/morkit/hooks/hooks-codex.json ~/.codex/hooks.json
   ```

   `hooks-codex.json` đã được fork sẵn với matcher phù hợp Codex (`apply_patch|shell|Edit|Write` thay vì Claude's `Skill`). Không edit `hooks/hooks.json` (Claude Code source) — nó không match Codex tool names.
...
`doctor-codex.sh` check: codex CLI version, skill symlink → `skills-codex/` + count ≥ 20, AGENTS.md, hooks state (`hooks-codex.json`), `commands-codex/` presence, drift check (`scripts/check-codex-drift.sh`), deep-review prereqs. Exit 0 = healthy, 1 = có FAIL.
...
| Slash commands `/morkit:X` | Native discovery từ `commands/` | Bridge qua AGENTS.md (model đọc `commands-codex/X.md`) |
| Skills location | `skills/` (auto-discover) | `skills-codex/` (fork — symlink target từ `~/.agents/skills/morkit`) |
| Skills auto-invoke | Native via Skill tool | Native via skill discovery |
| Hooks config | `hooks/hooks.json` (auto-load) | `hooks/hooks-codex.json` → wire vào `~/.codex/hooks.json` (qua `--with-hooks`) |
| Env var marker | `${CLAUDE_PLUGIN_ROOT}` | `${MORKIT_PLUGIN_ROOT}` (rewrite tự động trong `skills-codex/` + `commands-codex/`) |
| Subagents (deep-review specialists) | Native subagent dispatch | `codex exec` parallel wrapper (`scripts/codex-deep-review.sh`) |

## Troubleshooting

- **`skills-codex/ not found`** khi chạy installer hoặc doctor → checkout chưa có fork. Chạy:
  ```bash
  bash ~/.codex/morkit-source/plugins/morkit/scripts/sync-codex-fork.sh
  ```
  Sau đó re-run installer. Script này regenerate `skills-codex/` + `commands-codex/` từ `skills/` + `commands/` với vocab Claude→Codex.

- **Doctor báo "symlink expected skills-codex"** → symlink cũ trỏ vào `skills/`. Fix:
  ```bash
  rm ~/.agents/skills/morkit
  ln -s ~/.codex/morkit-source/plugins/morkit/skills-codex ~/.agents/skills/morkit
  ```

- **Doctor báo "drift"** → user edit trực tiếp `skills-codex/` hoặc `skills/` mà không re-sync. Chạy `scripts/check-codex-drift.sh` để xem file nào lệch, rồi `scripts/sync-codex-fork.sh` để regenerate fork.

- **Hooks không trigger** → kiểm tra `codex features list | grep codex_hooks` (phải enabled) và `ls -la ~/.codex/hooks.json` (phải trỏ vào `hooks-codex.json`, không phải `hooks.json` cũ).
