...
`morkit/changes/<name>/` (active) hoặc `morkit/changes/archive/<name>/` (đã archive).
...
Sau `/morkit:propose`, plugin sinh `morkit/changes/<name>/review-checklist.md` từ canonical Google Doc. Auto-detect variant (BE/FE × Feature/BugFix/Refactor).
...
Script chỉ `mv openspec/changes` → `morkit/changes`, không chạm `openspec/specs/` hay file khác.
