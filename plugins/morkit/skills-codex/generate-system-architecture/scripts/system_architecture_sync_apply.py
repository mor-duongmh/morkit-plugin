"""Step 2 of arch sync — convert checked proposal items into a Delta JSON.

Reads `arch-sync-proposal.md` produced by `system_architecture_sync_propose.py`,
extracts items whose checkbox is ticked (`[x]` or `[X]`), and writes a Delta
JSON for the standard update flow.

CLI:
    system_architecture_sync_apply.py --proposal .tmp/arch-sync-proposal.md \
...
_CHECKED_LINE = re.compile(
    r"^###\s+\[(?P<box>[ xX])\]\s+(?P<op>ADD|UPDATE|DEPRECATE)\s+(?P<id>CMP-[A-Za-z0-9_-]+)\s*$",
    re.MULTILINE,
)
# Capture "- Name: `name`" / "- Kind: x" for ADD payload
_NAME_BULLET = re.compile(r"^-\s*Name:\s*`([^`]+)`", re.MULTILINE)
_KIND_BULLET = re.compile(r"^-\s*Kind:\s*([a-zA-Z]+)", re.MULTILINE)
...
def parse_proposal(text: str) -> list[Change]:
    ...
    for i, m in enumerate(matches):
        ...
        km = _KIND_BULLET.search(block)
        if km:
            payload["kind"] = km.group(1)
