...
6b. **Refresh agent-instructions** (`references/agent-instructions.md`): rebuild the root `CLAUDE.md` (+ `AGENTS.md` if Codex) pointer block from the current docs. Expect state `[C]` — replace ONLY inside the marker; if the marker is gone, treat as `[B]` (append at end). No-op if unchanged. Approve gate per file.
