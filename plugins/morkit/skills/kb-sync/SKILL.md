...
```
- Collects PENDING tasks (folders in `knowledge/changes/` not yet in the ledger).
- Scope = union of `**Files:**` in each task's `tasks.md` (NOT git diff).
- Scans in-scope repos from SOURCE; diffs vs `catalog.json`.
- Writes a checkbox proposal to `.tmp/`. **Does NOT touch the KB.**
