...
- `[change-name]` — optional; otherwise pick the most recently modified change folder under `${MORKIT_ROOT:-morkit/changes}/` (excluding `archive/`).
...
1. **Resolve change directory.**
   ...
   - If a name was provided, set `CHANGE_DIR="${MORKIT_ROOT:-morkit/changes}/<name>"` and verify it exists.
   - Otherwise, find the most recent non-archive folder:
     ```bash
     ROOT="${MORKIT_ROOT:-morkit/changes}"
