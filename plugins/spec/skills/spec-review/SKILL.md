---
name: spec-review
description: Generate the developer review checklist (human gate) for an OpenSpec change, blocking implementation skills until the human flips "Overall Decision: OK". Triggered by /spec:review or after /spec:propose finishes.
license: MIT
---

Generate or refresh the **developer review checklist** for an OpenSpec change. This file is the human gate that the plugin's PreToolUse hook checks before allowing implementation skills to run.
...
- `[change-name]` — optional; otherwise pick the most recently modified change folder under `openspec/changes/` (excluding `archive/`).
...
1. **Resolve change directory.**
   ...
   - If a name was provided, set `CHANGE_DIR="openspec/changes/<name>"` and verify it exists.
   ...
   - If no change folder exists, stop and tell the user to run `/spec:propose` first.
...
3. **Report status to the user.**
   ...
   - A clear instruction to:
     ...
     5. Re-run `/spec:apply` (or `/superpowers:executing-plans` / `/superpowers:subagent-driven-development`) — the plugin's hook will allow it.
...
Next steps:
  ...
  4. Run /spec:apply (or /superpowers:executing-plans / /superpowers:subagent-driven-development)
